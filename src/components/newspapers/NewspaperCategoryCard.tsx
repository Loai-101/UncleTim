...
export type NewspaperFilterValue =
  | "newspaper"
  | "magazine"
  | "interview"
  | "report";

type NewspaperCategoryCardProps = {
  value: NewspaperFilterValue;
  labelKey: string;
  coverSrc: string;
  coverAlt: string;
  onOpen: () => void;
...
/**
 * Press category card — click opens the newspaper archive window.
 */
export function NewspaperCategoryCard({
  value,
  labelKey,
  coverSrc,
  coverAlt,
    ...
    >
      <div className="relative min-h-[11rem] flex-1 overflow-hidden bg-burgundy/10">
        ...
        <span className="absolute end-3 top-3 inline-flex size-9 items-center justify-center rounded-full border border-luxury-gold/60 bg-warm-white/90 text-burgundy opacity-0 transition-opacity duration-300 group-hover:opacity-100">
          <Newspaper className="size-4" strokeWidth={1.5} aria-hidden />
        </span>
      </div>
