...
import { getNewspaperImageAlt } from "@/data/seo/localized-alts";
import { cn } from "@/lib/utils";
import type { NewspaperArticle } from "@/types/gallery";

type NewspaperCoverCardProps = {
...
/**
 * Equal square press cover thumbnail — object-contain, centered, no crop.
 */
export function NewspaperCoverCard({
  article,
  onOpen,
  className,
}: NewspaperCoverCardProps) {
  const t = useTranslations("newspapers");
  const locale = useLocale();
  const alt = getNewspaperImageAlt(
    article.id,
    locale,
